...
function applyEstadosLSToIncoming(incoming) {
  ...
  return (incoming || []).map((o) => {
    ...
    const backendEsDefault = !backendEstado || backendEstado.toLowerCase() === "por preparar" || backendEstado === "-";
...
function estadoStyle(estado) {
  ...
  if (s.includes("cancelado") || s.includes("anulado") || s.includes("canceled") || s.includes("cancelled")) {
    return { label: "Cancelado", icon: "🛑", wrap: `${base} bg-rose-700 border-rose-800 text-white`, dot: `${dotBase} bg-rose-200` };
  }
...
function requiereImagenModificada(item) {
  ...
  const tieneCamposImagen =
    esImagenUrl(item?.image_original) || esImagenUrl(item?.image_url) || esImagenUrl(item?.imagen_original) || esImagenUrl(item?.imagen_url);
...
window.verDetalles = async function (orderId) {
  ...
  try {
    const url =
      typeof apiUrl === "function" ? apiUrl(`/dashboard/detalles/${encodeURIComponent(id)}`) : `/index.php/dashboard/detalles/${encodeURIComponent(id)}`;
    ...
    const clienteNombre = o.customer ? `${o.customer.first_name || ""} ${o.customer.last_name || ""}`.trim() : "";
    ...
    const itemsHtml = lineItems
      .map((item, index) => {
        ...
        const badgeText = estadoItem === "LISTO" ? "Listo" : estadoItem === "FALTA" ? "Falta imagen" : "Sin imagen";
...
window.renderUsersStatus = function (payload) {
  ...
  onlineEl.innerHTML = online.length ? online.map(renderUserRow("online")).join("") : `<li class="text-sm text-emerald-800/80">No hay usuarios conectados</li>`;

  offlineEl.innerHTML = offline.length ? offline.map(renderUserRow("offline")).join("") : `<li class="text-sm text-rose-800/80">No hay usuarios desconectados</li>`;
};

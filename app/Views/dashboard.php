...
<body class="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 overflow-x-hidden">
      ...
      <div class="mx-auto w-full max-w-[1400px]">
          ...
          <div class="rounded-3xl border border-slate-200 bg-white/80 backdrop-blur shadow-sm p-4 sm:p-6">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full lg:w-auto">
                ...
                <div class="rounded-2xl border border-slate-200 bg-white p-4">
                  <div class="text-[11px] uppercase tracking-wider text-slate-500">Estado general</div>
                  <div class="mt-2 flex items-center gap-2">
                    <span class="h-2.5 w-2.5 rounded-full bg-emerald-500"></span>
                    <span class="text-sm font-semibold text-slate-800">Panel activo</span>
                  </div>
                </div>
              </div>
          ...
          <div class="rounded-3xl border border-slate-200 bg-white shadow-sm p-4 sm:p-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div class="rounded-2xl border border-emerald-200 bg-emerald-50/50 p-4">
                ...
                <p class="mt-3 text-xs text-emerald-800/80">Verde = activo en los últimos 2 min</p>
              </div>
              ...
              <div class="rounded-2xl border border-rose-200 bg-rose-50/50 p-4">
                ...
                <p class="mt-3 text-xs text-rose-800/80">Rojo = sin actividad reciente</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section class="mt-5 sm:mt-6 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          <button id="btnAnterior"
                  disabled
                  class="w-full sm:w-auto px-4 py-3 rounded-2xl bg-slate-200 text-slate-800 font-semibold opacity-50 cursor-not-allowed transition">
            ← Anterior
          </button>
  ...
  <!-- MODAL DETALLES -->
  <div id="modalDetalles" class="hidden fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
    <div class="bg-white w-[94%] sm:w-[92%] h-[92%] rounded-3xl shadow-2xl p-4 sm:p-6 overflow-hidden flex flex-col animate-fadeIn">
      ...
      <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 border-b pb-4">
        <h2 id="tituloPedido" class="text-xl sm:text-2xl font-extrabold text-slate-900">Detalles del pedido</h2>
      ...
      <div id="detalleProductos" class="flex-1 overflow-auto grid grid-cols-1 lg:grid-cols-2 gap-4 p-2 sm:p-4"></div>
  ...
  <!-- PANEL CLIENTE -->
  <div id="panelCliente" class="hidden fixed inset-0 flex justify-end bg-black/30 backdrop-blur-sm z-50">
    <div class="w-[92%] sm:w-[420px] h-full bg-white shadow-xl p-4 sm:p-6 overflow-y-auto animate-fadeIn">
      ...
      <div class="flex justify-between items-center mb-4">
        ...
        <button onclick="cerrarPanelCliente()" class="text-slate-600 hover:text-slate-900 text-2xl font-bold">×</button>
      </div>
  ...
  <!-- LOADER GLOBAL -->
  <div id="globalLoader" class="hidden fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[9999]">

...
<body class="flex">
  ...
  <div class="flex-1 md:ml-64 p-8">
    <div class="card">
      ...
      <!-- (compat) grid plano, no se usa normalmente -->
  ...
  <script>
    ...
    function getLoteItemsFor(item) {
      ...
      if (!lid) return item ? [item] : [];
      return loteIndex[lid] || (item ? [item] : []);
    }
    ...
    function setSelectedItem(id) {
      ...
      // Preview grande: usa thumb solo para IMG (para PDF dejamos iframe)
      const bigImgSrc = it.url;

      q('modalPreview').innerHTML = isImg
        ? `<img src="${bigImgSrc}" style="width:100%;height:100%;object-fit:contain;">`
        : isPdf
      ...
      const loteNombre = (it.lote_nombre || '').trim();
    ...
    async function cargarStats() {
      try {
        ...
        if (data?.success) q('placasHoy').textContent = data.data?.total ?? data.total ?? 0;
      } catch (e) {}
    }
    ...
    async function cargarVistaAgrupada() {
      ...
      try {
        ...
        cont.innerHTML = `<div class="muted">Error cargando archivos</div>`;
        return;
      }

      if (data?.success) q("placasHoy").textContent = data.placas_hoy ?? data.data?.placas_hoy ?? q("placasHoy").textContent;
      ...
      const diasFiltrados = data.dias
        .map(dia => {
          ...
          return okDia
            ? dia
            : { ...dia, lotes, total_archivos: lotes.reduce((a,l)=>a+(l.items?.length||0),0) };
        })
      ...
      for (const dia of dias) {
        ...
        for (const lote of (dia.lotes || [])) {
          ...
          const principal = items.find(x => Number(x.is_primary) === 1) || items[0];
          const thumb = principal
            ? (principal.thumb_url || ((principal.mime || "").startsWith("image/") ? principal.url : null))
            : null;
          ...
          loteBox.innerHTML = `
            <div class="lote-left">
              ...
              <div class="min-w-0">
                ...
                <div class="lote-meta">${total} archivo(s) • ${escapeHtml(formatFecha(lote.created_at ?? ""))}</div>
              </div>
            </div>
          ...
          loteBox.onclick = () => window.openLote(lid);
    ...
    async function renombrarLoteDesdeModal() {
      ...
      window.openModal(keepId);
    }

    // Eventos modal (una sola vez)
    ...
    q('btnGuardarCarga').addEventListener('click', () => {
      const producto = q('cargaProducto')?.value?.trim() || ''; // si no existe, queda vacío
      ...
      xhr.onload = async () => {
        ...
        setTimeout(async () => {
          ...
          // abrir lote nuevo si viene en respuesta; si no, buscar por nombre
          const newLoteId = data.lote_id || data.data?.lote_id || null;
          if (newLoteId) {
            window.openLote(String(newLoteId));
          } else {
            const buscado = loteNombreManual;
            ...
            if (found) window.openLote(found);
          }
        }, 200);
      };

...
class PlacasArchivosController extends BaseController
{
    ...
    public function listar()
    {
        ...
        foreach ($items as &$it) {
            $it['url'] = base_url($it['ruta']); // ej: https://dominio.com/uploads/placas/xxx.png
    ...
    public function subir()
    {
        ...
        // 1) Multi (archivos[]) o single (archivo)
        $lista = $this->request->getFileMultiple('archivos'); // para <input multiple name="archivos[]">
        if (empty($lista)) {
            $single = $this->request->getFile('archivo');     // compatibilidad
